...
def do_macro(parser, token):
    ...
    for arg in args:
        if not r_valid_arg_name.match(arg):
            raise template.TemplateSyntaxError('Argument "{0}" to macro "{1}" '
                'contains illegal characters. Only alphanumeric characters '
                'and "_" are allowed.'.format(arg, macro_name))
...
class UseMacroNode(template.Node):
    ...
    def render(self, context):
        for i, arg in enumerate(self.args):
            context[arg] = self.filter_expressions[i].resolve(context)
